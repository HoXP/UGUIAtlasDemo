...
{
    public SheetImporter(string selectPath, string tpOUT, Dictionary<string, Vector4> borderInfo)
    {
    ...
    private void DeserialiseXML()
    {
        ...
        string __xmlString = (string)FileManager.ReadAllText(__path).data;
        ...
        for (int i = 0; i < __elm.ChildNodes.Count; i++)
        {
            ...
            SpriteMetaData __data = new SpriteMetaData();
            __data.name = __child.GetAttribute("name");
            __data.border = _savedBorderInfo != null && _savedBorderInfo.ContainsKey(__data.name) ? _savedBorderInfo[__data.name] : Vector4.zero;
    ...
    private Dictionary<string, Vector4> _savedBorderInfo;
}
